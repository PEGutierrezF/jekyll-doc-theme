...
####  Before we can use this explanatory matrix we need to check that its rows are in the same order as our response matrix
...
####  Transformar datos.
Hellinger es una transformacion recomendada por Legendre & Callagher (2001) en datos de abundancia y con una respuesta lineal

```{r}

spec.hel <- vegan::decostand(species_1, method = "hellinger")
...
```{r}
rda_Forward_Sel <- rda(spec.hel~1, data= env_1)
rda_var <- rda(spec.hel ~ temperature + oxygen + pH +
...
rda_tree_all = rda(spec.hel ~ temperature + oxygen + pH +
                conductivity, data= env_1, scale=T)
